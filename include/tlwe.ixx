...
export template <class P, std::make_signed_t<typename P::T> μ>
std::vector<TLWE<P>> bootsSymEncrypt(const std::vector<uint8_t> &p,
                                     const Key<P> &key)
{
    std::vector<TLWE<P>> c(p.size()); // Use std::vector
...
export template <class P = lvl1param, std::make_signed_t<typename P::T> μ>
std::vector<TLWE<P>> bootsSymEncrypt(const std::vector<uint8_t> &p,
                                     const SecretKey &sk)
{
...
std::vector<uint8_t> bootsSymDecrypt(const std::vector<TLWE<P>> &c,
                                     const Key<P> &key)
{
    std::vector<uint8_t> p(c.size()); // Use std::vector
#pragma omp parallel for
