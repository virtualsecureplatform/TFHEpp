export module tfhepp:utils;
import std;

#if defined(USE_BLAKE3)
export import tfhepp:BLAKE3PRNG;
#elif defined(USE_RANDEN)
#import <randen.h> // External library, needs special handling
#endif

// Polynomial type is used in some function signatures
export import tfhepp:params; // This should provide Polynomial type alias and P::n, P::nbit etc.

export namespace TFHEpp {
#if defined(USE_BLAKE3)
static thread_local std::random_device trng;
export static thread_local TFHEpp::BLAKE3PRNG::BLAKE3PRNG<uint64_t> generator(trng); // Qualified with TFHEpp::
#elif defined(USE_RANDEN)
static thread_local std::random_device trng;
export static thread_local randen::Randen<uint64_t> generator(trng); // randen:: might need to be TFHEpp::randen if wrapped
#else
export static thread_local std::random_device generator; // Exporting a generator like this might be problematic.
#endif
...
export template <typename T, std::size_t alignment>
...
export inline uint16_t dtot16(double d)
{
    return int16_t(int64_t((d - int64_t(d)) * (1LL << 16)));
}

// Double to Torus(32bit fixed-point number)
export inline uint32_t dtot32(double d)
{
