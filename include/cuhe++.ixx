export module tfhepp:cuhe++;
import std;

export import tfhepp:INTorus;
// Assuming P (cuHEpp::P) is exported by INTorus.ixx or a params module imported by INTorus.ixx
// export import tfhepp:params; // If P is directly from params and not re-exported by INTorus
...
export constexpr uint64_t W = 12037493425763644479ULL;
...
export inline void ButterflyAdd(INTorus *const res, const uint32_t size)
{
...
export template <uint8_t radixbit>
...
export template <uint32_t Nbit, uint8_t radixbit>
...
export template <typename T = uint32_t, uint32_t Nbit>
            ...
            res[i] =
                INTorus(
                    static_cast<uint64_t>(
                        ((static_cast<__uint128_t>(a[i]) * cuHEpp::P) + (1ULL << 63)) >> // Assuming P is cuHEpp::P from INTorus.ixx
                        64),
                    true) *
                twist[i];
    }
...
export template <typename T, uint32_t Nbit>
                           ...
                           const std::array<INTorus, 1 << Nbit> &twist)
{
    ...
    if constexpr (std::is_same_v<T, uint64_t>) {
        for (int i = 0; i < N; i++) {
            ...
            //      << 64) /
            //     cuHEpp::P); // Assuming P is cuHEpp::P
        }
    }
    else {
        for (int i = 0; i < N; i++)
