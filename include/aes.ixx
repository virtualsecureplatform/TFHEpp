export module tfhepp:aes;
import std;
#import <AES.h> // Noted: This is likely a non-standard C header and will need special handling.

// Transciphering by AES
//  Based on Hippogryph
export namespace TFHEpp {

export constexpr uint aesNb = 4;

export void SubWord(std::array<uint8_t,4>& a) {
  int i;
  for (i = 0; i < 4; i++) {
...
export void RotWord(std::array<uint8_t,4>& a) {
  uint8_t c = a[0];
  a[0] = a[1];
  a[1] = a[2];
  a[2] = a[3];
...
export inline uint8_t xtime(uint8_t b)  // multiply on x
{
  return (b << 1) ^ (((b >> 7) & 1) * 0x1b);
}

export uint8_t Rcon(unsigned int n) {
  uint8_t c = 1;
...
export constexpr uint Nk = 4;  // Number of 32-bit words in the key
export constexpr uint Nb = 4;  // Number of columns (32-bit words) comprising the state
export constexpr uint Nr = 10; // Number of rounds, which is a function of Nk and Nb

export void KeyExpansion(std::array<uint8_t, 4*Nb*(Nr+1)>& w, const std::array<uint8_t,16>& key) {
...
export template <class iksP, class brP>
...
export template <class iksP, class brP, class cbiksP, class cbbrP>
void SubBytes(std::array<TLWE<typename brP::targetP>, 128>& state,
                const EvalKey &ek)
{
    for(int i = 0; i < 16; i++){
...
export template <class P, uint index, uint n>
...
export uint8_t Rcon(const uint8_t n){ // Overloaded Rcon
    uint8_t rcon = 1;
    for (int i = 0; i < n - 1; i++) {
        rcon = xtime(rcon);
    }
...
export template <class iksP, class brP>
void KeyExpansion(std::array<std::array<TLWE<typename brP::targetP>, 128>, 10> &expandedkey, // Note: Nr (10) is used here
                  const std::array<TLWE<typename iksP::domainP>, 128> &key,
                  EvalKey &ek)
{
    KeyExpand<iksP, brP>(expandedkey[0], key, ek, 1);
    for (int i = 1; i < Nr; i++) // Use Nr for the loop limit
