export module tfhepp:fnt;
import std;

export namespace FNTpp {
export constexpr unsigned int Kbit = 5;
export constexpr unsigned int K = 1 << Kbit;
export constexpr int64_t P = (1ULL << K) + 1;
export constexpr int64_t wordmask = (1ULL << K) - 1;
...
export static inline int64_t ModLshift(int64_t a, uint8_t b)
{
...
export template <unsigned int Nbit>
...
{
    if constexpr (Nbit <= Kbit) {
        // res = a; // This was incorrect, should copy only relevant part if sizes differ
        for(unsigned int i = 0; i < (1u << Nbit); ++i) res[i] = a[i]; // Assuming a might be larger
