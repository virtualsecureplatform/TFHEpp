...
struct alignas(64) BLAKE3PRNG{
  ...
  uint64_t counter = 0; // counter for the number of times the buffer has been filled
  ...
  result_type operator()() {
    ...
    if (next == buffer_result_len) {
        counter++;
        blake3_hasher_finalize_seek(&hasher, counter*buffer_result_len, (uint8_t*)buffer.data(), buffer_len);
        next = 0;
    }
