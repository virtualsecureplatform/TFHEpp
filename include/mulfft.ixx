export module tfhepp:mulfft;
import std;

export import tfhepp:INTorus;
...
#ifdef USE_HEXL
// This likely needs to be handled with a proper module import if HEXL supports it,
// or via a global module fragment / separate compilation for HEXL parts.
#import "hexl/hexl.hpp"
#endif

#ifdef USE_INTERLEAVED_FORMAT
// <complex> is part of `import std;`
#endif

export import tfhepp:cuhe++;
export import tfhepp:params;
import tfhepp:utils;

export namespace TFHEpp {

export inline const std::unique_ptr<
    const std::array<std::array<TFHEpp::cuHEpp::INTorus, lvl1param::n>, 2>> // TFHEpp::cuHEpp needed
    ntttwistlvl1 = TFHEpp::cuHEpp::TwistGen<lvl1param::nbit>(); // TFHEpp::cuHEpp needed
export inline const std::unique_ptr<
    const std::array<std::array<TFHEpp::cuHEpp::INTorus, lvl1param::n>, 2>> // TFHEpp::cuHEpp needed
    ntttablelvl1 = TFHEpp::cuHEpp::TableGen<lvl1param::nbit>(); // TFHEpp::cuHEpp needed
export inline const std::unique_ptr<
    const std::array<std::array<TFHEpp::cuHEpp::INTorus, lvl2param::n>, 2>> // TFHEpp::cuHEpp needed
    ntttwistlvl2 = TFHEpp::cuHEpp::TwistGen<lvl2param::nbit>(); // TFHEpp::cuHEpp needed
export inline const std::unique_ptr<
    const std::array<std::array<TFHEpp::cuHEpp::INTorus, lvl2param::n>, 2>> // TFHEpp::cuHEpp needed
    ntttablelvl2 = TFHEpp::cuHEpp::TableGen<lvl2param::nbit>(); // TFHEpp::cuHEpp needed
// export inline const std::unique_ptr< // raintt types might not be defined yet
...
export constexpr uint64_t lvl1P = 1073707009;
#endif
...
inline void TwistNTT(Polynomial<P> &res, PolynomialNTT<P> &a)
{
...
#ifdef USE_HEXL
    {
        ...
        static intel::hexl::NTT nttlvl1(lvl1param::n, lvl1P); // Needs USE_HEXL and intel::hexl defined
        nttlvl1.ComputeInverse(temp.data(), &(a[0].value), 1, 1);
        for (int i = 0; i < lvl1param::n; i++) res[i] = (temp[i] << 32) / lvl1P;
    }
#else
        TFHEpp::cuHEpp::TwistNTT<typename lvl1param::T, lvl1param::nbit>( // TFHEpp::cuHEpp needed
            res, a, (*ntttablelvl1)[0], (*ntttwistlvl1)[0]);
#endif
    else if constexpr (std::is_same_v<typename P::T, uint64_t>) {
        TFHEpp::cuHEpp::TwistNTT<typename lvl2param::T, lvl2param::nbit>( // TFHEpp::cuHEpp needed
            res, a, (*ntttablelvl2)[0], (*ntttwistlvl2)[0]);
    }
    else
        static_assert(::TFHEpp::cuHEpp::false_v<typename P::T>, "Undefined TwistNTT!"); // TFHEpp::cuHEpp::false_v if it's from there
}
...
inline void TwistFFT(Polynomial<P> &res, const PolynomialInFD<P> &a)
{
    if constexpr (std::is_same_v<P, lvl1param>) {
        if constexpr (std::is_same_v<typename P::T, uint32_t>)
            ...
            // else
            fftplvl1.execute_direct_torus32(res.data(), a.data()); // fftplvl1 needs definition
        if constexpr (std::is_same_v<typename P::T, uint64_t>)
            fftplvl1.execute_direct_torus64(res.data(), a.data()); // fftplvl1 needs definition
    }
    else if constexpr (std::is_same_v<typename P::T, uint64_t>)
        fftplvl2.execute_direct_torus64(res.data(), a.data()); // fftplvl2 needs definition
    else
        static_assert(::TFHEpp::cuHEpp::false_v<typename P::T>, "Undefined TwistFFT!");
}
...
#else
        TFHEpp::cuHEpp::TwistINTT<typename P::T, P::nbit>(res, a, (*ntttablelvl1)[1], // TFHEpp::cuHEpp needed
                                                  (*ntttwistlvl1)[1]);
#endif
    else if constexpr (std::is_same_v<typename P::T, uint64_t>)
        TFHEpp::cuHEpp::TwistINTT<typename P::T, P::nbit>(res, a, (*ntttablelvl2)[1], // TFHEpp::cuHEpp needed
                                                  (*ntttwistlvl2)[1]);
    else
        static_assert(::TFHEpp::cuHEpp::false_v<typename P::T>, "Undefined TwistINTT!");
}
...
inline void TwistIFFT(PolynomialInFD<P> &res, const Polynomial<P> &a)
{
    if constexpr (std::is_same_v<P, lvl1param>) {
        ...
        static_assert(::TFHEpp::cuHEpp::false_v<typename P::T>, "Undefined TwistIFFT!");
}
                                   ...
                                   const Polynomial<P> &b)
{
    ...
    // }
    // else
    //     static_assert(::TFHEpp::cuHEpp::false_v<typename P::T>, "Undefined PolyMul!");
}
...
// These need to be exported if used by other modules.
// If xaittlvl1/2 are implementation details, they could be in an unnamed namespace or a detail module.
// For now, assuming they might be needed by other parts of tfhepp if USE_TERNARY or USE_KEY_BUNDLE is defined.
export alignas(64) static const std::unique_ptr< // static const might be problematic for export
                                           ...
                                           const int a)
{
    ...
    if constexpr (std::is_same_v<P, lvl1param>) {
        MulInFD<P::n>(res, poly, (*xaittlvl1)[index]); // This will cause issues if xaittlvl1 is not truly exported or visible.
    }
}
#endif
}  // namespace TFHEpp