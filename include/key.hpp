...
struct lweKey {
    ...
    lweKey(){
        ...
        for (typename lvl0param::T &i : std::get<Key<lvl0param>>(keys)) i = lvl0gen(generator);
        for (typename lvlhalfparam::T &i : std::get<Key<lvlhalfparam>>(keys)) i = lvlhalfgen(generator);
        for (typename lvl1param::T &i : std::get<Key<lvl1param>>(keys)) i = lvl1gen(generator);
        for (typename lvl2param::T &i : std::get<Key<lvl2param>>(keys)) i = lvl2gen(generator);
    #ifdef USE_SUBSET_KEY
        for (int i = 0; i < lvl1param::k * lvl1param::n; i++) std::get<Key<lvl2param>>(keys)[i] = std::get<Key<lvl1param>>(keys)[i];
    #endif
    }
    template <class P>
    Key<P> get() const
