...
export import tfhepp:cuhe++; // For cuHEpp::BitReverse
                         ...
                         const Polynomial<P> &poly)
{
    ...
    Decomposition<P>(decpoly, poly);
    for (int i = 0; i < P::l; i++)
        TFHEpp::raintt::TwistINTT<typename P::T, P::nbit, false>( // TFHEpp::raintt
            decpolyntt[i], decpoly[i], (*TFHEpp::raintttable)[1], (*TFHEpp::raintttwist)[1]); // TFHEpp::
}
                                ...
                                const TRGSWRAINTT<P> &trgswntt)
{
    ...
    // TFHEpp::raintt::TwistNTT<typename P::T,P::nbit, P::q == (1ULL<<P::qbit)>(res[k], // TFHEpp::raintt
    // restrlwentt[k],(*TFHEpp::raintttable)[0],(*TFHEpp::raintttwist)[0]); // TFHEpp::
    for (int k = 0; k < P::k + 1; k++)
        TFHEpp::raintt::TwistNTT<typename P::T, P::nbit, true>( // TFHEpp::raintt
            res[k], restrlwentt[k], (*TFHEpp::raintttable)[0], (*TFHEpp::raintttwist)[0]); // TFHEpp::
}
...
TRGSWRAINTT<P> ApplyRAINTT2trgsw(const TRGSW<P> &trgsw)
{
    ...
    for (int i = 0; i < P::k * P::lₐ + P::l; i++)
        for (int j = 0; j < P::k + 1; j++) {
            TFHEpp::raintt::TwistINTT<typename P::T, P::nbit, true>( // TFHEpp::raintt
                trgswntt[i][j], trgsw[i][j], (*TFHEpp::raintttable)[1], // TFHEpp::
                (*TFHEpp::raintttwist)[1]); // TFHEpp::
            for (int k = 0; k < P::n; k++)
                if ((k & ((1 << remainder) - 1)) > 1)
                    trgswntt[i][j][k] =
                        TFHEpp::raintt::MulSREDC(trgswntt[i][j][k], TFHEpp::raintt::R4); // TFHEpp::raintt
                else
                    trgswntt[i][j][k] =
                        TFHEpp::raintt::MulSREDC(trgswntt[i][j][k], TFHEpp::raintt::R2); // TFHEpp::raintt
        }
    return trgswntt;
}
    ...
    for (int i = 0; i < P::k * P::lₐ + P::l; i++)
        for (int j = 0; j < P::k + 1; j++) {
            ...
            for (uint32_t k = 0; k < P::n; k++)
                trgswntt[i][j][k] = temp[TFHEpp::cuHEpp::BitReverse<P::nbit>(k)]; // TFHEpp::cuHEpp
        }
    return trgswntt;
}
...
TRGSWRAINTT<P> trgswrainttSymEncrypt(const Polynomial<P> &p, const uint η,
                                     const Key<P> &key)
{
    if constexpr (hasq<P> && P::q == TFHEpp::raintt::P) { // TFHEpp::raintt
        constexpr uint8_t remainder = ((P::nbit - 1) % 3) + 1;
        ...
        for (TRLWERAINTT<P> &trlweraintt : trgswraintt) {
            ...
            for (int k = 0; k <= P::k; k++)
                for (int j = 0; j < P::n; j++)
                    trlweraintt[k][j] = TFHEpp::raintt::MulSREDC( // TFHEpp::raintt
                        trlweraintt[k][j], ((j & ((1 << remainder) - 1)) > 1)
                                               ? TFHEpp::raintt::R3 // TFHEpp::raintt
                                               : TFHEpp::raintt::R2); // TFHEpp::raintt
        }
        for (int i = 0; i < P::lₐ; i++) {
            ...
            for (int k = 0; k < P::k; k++) {
                ...
                TFHEpp::raintt::TwistINTT<typename P::T, P::nbit, false>( // TFHEpp::raintt
                    praintt, pscaled, (*TFHEpp::raintttable)[1], (*TFHEpp::raintttwist)[1]); // TFHEpp::
                for (int j = 0; j < P::n; j++)
                    if ((j & ((1 << remainder) - 1)) > 1)
                        praintt[j] = TFHEpp::raintt::MulSREDC(praintt[j], TFHEpp::raintt::R4); // TFHEpp::raintt
                    else
                        praintt[j] = TFHEpp::raintt::MulSREDC(praintt[j], TFHEpp::raintt::R2); // TFHEpp::raintt
                for (int j = 0; j < P::n; j++)
                    trgswraintt[i + k * P::lₐ][k][j] = TFHEpp::raintt::AddMod( // TFHEpp::raintt
        ...
        for (int i = 0; i < P::l; i++) {
            ...
            for (int j = 0; j < P::n; j++)
                if ((j & ((1 << remainder) - 1)) > 1)
                    ...
                    praintt[j] = TFHEpp::raintt::MulSREDC(praintt[j], TFHEpp::raintt::R2); // TFHEpp::raintt
            for (int j = 0; j < P::n; j++)
                trgswraintt[i + P::k * P::lₐ][P::k][j] = TFHEpp::raintt::AddMod( // TFHEpp::raintt
