...
struct EvalKey {
    ...
    std::tuple<
        ...
        std::unordered_map<std::string, std::shared_ptr<PrivateKeySwitchingKey<lvl11param>>>, // 20
        std::unordered_map<std::string, std::shared_ptr<PrivateKeySwitchingKey<lvl21param>>>, // 21
        std::unordered_map<std::string, std::shared_ptr<PrivateKeySwitchingKey<lvl22param>>>, // 22
        // SubsetPrivateKeySwitchingKey
        std::unordered_map<std::string, std::shared_ptr<SubsetPrivateKeySwitchingKey<lvl21param>>>, // 23
        // AnnihilateKey
    ...
    template <typename T>
    auto& get_map() {
        return std::get<std::unordered_map<std::string, std::shared_ptr<T>>>(keys);
    }
    ...
    void emplacecbsk(const SecretKey& sk)
    {
        ...
        for (int i = 0; i < P::k; i++) {
            ...
            (*get<CBswitchingKey<P>>())[i] = trgswfftSymEncrypt<P>(partkey, sk.key.get<P>());
        }
    }
    ...
    {
        return *(const_cast<EvalKey*>(this)->get_map<PrivateKeySwitchingKey<P>>().at(key));
    }
    template <class P>
    SubsetPrivateKeySwitchingKey<P>& getsubprivksk(const std::string& key) const
    {
        return *(const_cast<EvalKey*>(this)->get_map<SubsetPrivateKeySwitchingKey<P>>().at(key));
    }
    template <class P>
