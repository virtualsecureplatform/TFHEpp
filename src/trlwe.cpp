...
namespace TFHEpp{
    ...
    array<array<uint32_t,DEF_N>,2>trlweSymEncryptZerolvl1(const double α, const array<uint32_t,DEF_N> &key){
    ...
    array<bool,DEF_N> trlweSymDecryptlvl1(const array<array<uint32_t,DEF_N>,2> &c, const array<uint32_t,DEF_N> &key){
    ...
    inline void SampleExtractIndex(array<T,N+1> &tlwe ,const array<array<T,N>,2> &trlwe, const int index){
        for(int i = 0; i <= index; i++) tlwe[i] = trlwe[0][index - i];
        for(int i = index+1; i < N; i++) tlwe[i] = trlwe[0][N + index - i];
        tlwe[N] = trlwe[1][index];
    }

    void SampleExtractIndexlvl1(array<uint32_t,DEF_N+1> &tlwe,const array<array<uint32_t,DEF_N>,2> &trlwe, const int index){
