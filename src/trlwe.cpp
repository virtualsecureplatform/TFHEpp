...
namespace TFHEpp{
    ...
    array<T,N+1> SampleExtractIndex(array<array<T,N>,2> trlwe, int index){
        array<T,N+1> tlwe;
    ...
    array<uint32_t,DEF_N+1> SampleExtractIndexlvl1(array<array<uint32_t,DEF_N>,2> trlwe, int index){
        return SampleExtractIndex<uint32_t,DEF_N>(trlwe,index);
    }
}