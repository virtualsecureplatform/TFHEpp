...
namespace TFHEpp{
    ...
    array<T,n+1> tlweSymEncrypt(const T p,const double α, const array<uint32_t,n> key){
        uniform_int_distribution<uint32_t> Torusdist(0, numeric_limits<T>::max());
    ...
    array<uint32_t,DEF_n+1> tlweSymEncryptlvl1(const uint32_t p, const array<uint32_t,DEF_n> key){
        return tlweSymEncrypt<uint32_t,DEF_n>(p,DEF_α,key);
    }
    ...
    vector<array<uint32_t,DEF_n+1>> bootsSymEncrypt(const vector<bool> p,const lweKey sk){
        ...
        for(int i = 0;i<p.size();i++) c[i] = tlweSymEncryptlvl1(p[i]?DEF_MU:-DEF_MU, sk.lvl0);
        return c;
    }

    vector<bool> bootsSymDecrypt(const vector<array<uint32_t,DEF_n+1>> c, const lweKey sk){
        vector<bool> p(c.size());
        for(int i = 0;i<p.size();i++) p[i] = tlweSymDecryptlvl1(c[i], sk.lvl0);
        return p;
    }
}