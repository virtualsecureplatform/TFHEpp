...
void PriKeySwitchlvl21(TRLWElvl1 &res, const TLWElvl2 &tlwe, const int u, PrivKeySwitchKey privksk){
    const uint32_t mask = (1<<DEF_basebitlvl21) - 1;
    const uint64_t prec_offset = 1UL<<(64-(1+DEF_basebitlvl21*DEF_tbar));
    ...
    for(int i = 0;i<=DEF_nbar;i++){
        ...
        for(int j = 0;j<DEF_tbar;j++){
            const uint64_t aij = (aibar>>(64-(j+1)*DEF_basebitlvl21)) & mask;
