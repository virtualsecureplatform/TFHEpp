                ...
                const SecretKey& sk)
{
    ...
    for (int i = 0; i < P::domainP::k*P::domainP::n; i++) key[i] = sk.key.get<typename P::domainP>()[i];
...
void EvalKey::emplaceprivksk4cb(const SecretKey& sk)
{
    for(int k = 0; k < P::targetP::k; k++){
        ...
        for(int i = 0; i<P::targetP::n; i++) partkey[i] = -sk.key.get<typename P::targetP>()[k*P::targetP::n+i];
        emplaceprivksk<P>("privksk4cb_" + std::to_string(k), partkey, sk);
    }
    emplaceprivksk<P>("privksk4cb_" + std::to_string(P::targetP::k), {1}, sk);
}
#define INST(P)                                                              \
    template void EvalKey::emplaceprivksk4cb<P>(                                \
        const SecretKey& sk)
TFHEPP_EXPLICIT_INSTANTIATION_KEY_SWITCH_TO_TRLWE(INST)
#undef INST
