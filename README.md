...
# TFHEpp
TFHEpp is a full scratched pure C++ version of TFHE. TFHEpp is slightly(about 10%) faster than the original [TFHE implementation](https://github.com/tfhe/tfhe). In addition to that, THFEpp supports [Circuit Bootstrapping](https://eprint.iacr.org/2018/421), [Programable Boootstrapping many LUT](https://eprint.iacr.org/2021/729), Partial Key([Klemsa's version](https://eprint.iacr.org/2021/634), [Zama's version](https://eprint.iacr.org/2023/979) . Named as subset key in the code), [Modified Chen's Packing](https://eprint.iacr.org/2024/1318) (We call it as annihilate key switching in our code), [mean compensation in rounding noises](https://eprint.iacr.org/2025/809), [Double Decomposition](https://eprint.iacr.org/2023/771) (bivariate polynomial representation) with 128-bit Torus support, and block-binary key parameters and block-binary blind rotation based on ["Faster TFHE Bootstrapping with Block Binary Keys"](https://eprint.iacr.org/2023/958).
We also include partial support for B/FV, written in include/bfv/bfv++.hpp. For the implementation of the Modified Chen's Packing, we also used the idea of [dividing TRLWE at each recursion](https://eprint.iacr.org/2025/1088), though I developed that independently.
Experimental Gentry–Lee complex matrix FHE and low-depth SHIP bootstrapping are available in `include/gl/`; see `docs/GL.md` for the Double Decomposition design, API, parameter-capacity warning, and current performance limitations.
