                ...
                const KeySwitchingKey<lvl10param> &ksk)
{
    ...
    PolynomialMulByXaiMinusOne<lvl1param>(temp[0], data[0],
                                   2 * lvl1param::n - (lvl1param::n >> 1));
    PolynomialMulByXaiMinusOne<lvl1param>(temp[1], data[1],
                                   2 * lvl1param::n - (lvl1param::n >> 1));
...
int main()
{
    ...
    for (uint8_t wrflag = 0; wrflag <= 1; wrflag++) {
        for (uint8_t msbaddress = 0; msbaddress <= 1; msbaddress++) {
            ...
            for (int i = 0; i < words; i++)
                HomMUXwoSE<lvl01param>(writed[i], cs, encwritep[i], encramreadres[i],
                           (*ck).gk.bkfftlvl01);
