...
template<uint32_t Nbit,uint8_t radixbit>
    void NTTdebug(std::array<INTorus,1<<Nbit > &res, const std::array<INTorus,1<<Nbit> &table, std::array<std::array<INTorus,1<<Nbit>,Nbit/radixbit+((Nbit%radixbit)>0?1:0)> &debug){
...
int main()
{
    ...
    const std::array<std::array<cuHEpp::INTorus,TFHEpp::lvl1param::n>,2> twistlvl1 = cuHEpp::TwistGen<TFHEpp::lvl1param::nbit>();
    const std::array<std::array<cuHEpp::INTorus,TFHEpp::lvl1param::n>,2> tablelvl1 = cuHEpp::TableGen<TFHEpp::lvl1param::nbit>();
    ...
    for(int i = 0;i<TFHEpp::lvl1param::n;i++) assert((tablelvl1[0][i]*tablelvl1[1][i]).value==1);
    for(int i = 1;i<TFHEpp::lvl1param::n;i++) assert(tablelvl1[0][i].value!=1);
    ...
    //     std::array<cuHEpp::INTorus,TFHEpp::lvl1param::n> res1,res2;
    ...
    //     const cuHEpp::INTorus shifter = cuHEpp::INTorus(1)<<(TFHEpp::lvl1param::nbit-radixbit);
    //     for (int i = 0; i < radix; i++) res[i] = cuHEpp::INTorus(a[i]);
    ...
    //     const std::array<std::array<cuHEpp::INTorus,size>,2> table = cuHEpp::TableGen<sizebit>();
    ...
    //     std::array<std::array<cuHEpp::INTorus,TFHEpp::lvl1param::n>,TFHEpp::lvl1param::nbit> tdebug;
    //     TFHEpp::Polynomial<TFHEpp::lvl1param> a;
    //     for (uint32_t &i : a) i = Bgdist(engine);
    ...
    //     cuHEpp::NTTdebug<TFHEpp::lvl1param::nbit,1>(tres, tablelvl1[0],tdebug);
    //     cuHEpp::NTTdebug<TFHEpp::lvl1param::nbit,3>(res, tablelvl1[0],debug);
    //     for(int i = 0;i<TFHEpp::lvl1param::n;i++) assert(debug[0][i].value==tdebug[0][i].value);
    //     for(int stage = 0; stage<3; stage++){
    //         std::cout<<stage<<std::endl;
    //         std::cout<<debug[2][186].value<<":"<<debug[2][250].value<<std::endl;
    //         for(int i = 0;i<TFHEpp::lvl1param::n;i++) if(debug[stage+1][i].value!=tdebug[3*(stage+1)][i].value) std::cout<<"index:"<<i<<",value:"<<debug[stage+1][i].value<<":"<<tdebug[3*(stage+1)][i].value<<std::endl;
    //         for(int i = 0;i<TFHEpp::lvl1param::n;i++) assert(debug[stage+1][i].value==tdebug[3*(stage+1)][i].value);
    //     }
    // }
    ...
    //     const std::array<uint64_t,1<<radixbit> a = {768,4294967295,13070072148996651454,2891126649239098500,17210682317612006867,17933073005031899459,15032082446739191299,6595323810082472591};
    //     const std::array<uint64_t,1<<radixbit> tres = {0,0,1024,1024,1024,0,1024,2048};
    //     const std::array<uint64_t,1<<radixbit> fres = {1152903912152367104,17293840157262217217,1152903912152368128,17293840157262218241,1152903912152368128,17293840157262217217,1152903912152368128,17293840157262219265};
    //     std::array<cuHEpp::INTorus, radix> res,ires;
    //     for (int i = 0; i < radix; i++) res[i] = cuHEpp::INTorus(a[i],false);
    //     for (uint32_t i = 1; i < radix; i++) res[i] *= tablelvl1[0][cuHEpp::BitReverse<radixbit>(i)*leading];

    //     for (int i = 0; i < radix; i++) ires[i] = cuHEpp::INTorus(tres[i]/radix,false);
    //     cuHEpp::INTTradixButterfly<radixbit>(ires.data(), radix);
    //     for(int i = 0; i<radix;i++) std::cout<<res[i].value<<":"<<ires[i].value<<":"<<tablelvl1[0][cuHEpp::BitReverse<radixbit>(i)*leading].value<<":"<<tablelvl1[1][cuHEpp::BitReverse<radixbit>(i)*leading].value<<":"<<(tablelvl1[0][cuHEpp::BitReverse<radixbit>(i)*leading]*tablelvl1[1][cuHEpp::BitReverse<radixbit>(i)*leading]).value<<std::endl;
    //     for(int i = 0;i<radix;i++) assert(res[i].value == ires[i].value);

    //     cuHEpp::NTTradixButterfly<radixbit>(res.data(), radix);

    //     for(int i = 0; i<radix;i++) std::cout<<res[i].value<<":"<<tres[i]<<":"<<fres[i]<<std::endl;
