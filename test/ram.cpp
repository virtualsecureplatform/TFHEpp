...
int main()
{
    ...
    for(int i = 0;i<memsize;i++){
        ...
        for(int j =0;j<address_bit;j++) CMUXFFTlvl1(temp,(*bootedTGSW)[addressbitset[j]][j],temp,encmemory[i]);
        TLWElvl1 temp2;
    ...
    array<bool,DEF_N> pwriteres = trlweSymDecryptlvl1(encmemory[addressint],(*sk).key.lvl1);
    assert(static_cast<int>(pwriteres[0]) == static_cast<int>((wrflag>0)?writep:pmemory[addressint]));
