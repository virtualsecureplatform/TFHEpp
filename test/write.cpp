...
void writeMUX(array<TRLWElvl1,(1<<address_bit)> &res, const array<array<TRGSWFFTlvl1,address_bit-1>,2> &address){
    trgswfftExternalProductlvl1(res[block+(1<<bitpos)],res[block],address[0][bitpos]);
...
int main()
{
    ...
    array<array<TRGSWFFTlvl1,address_bit-1>,2> *bootedTGSW = new array<array<TRGSWFFTlvl1,address_bit-1>,2>;
    ...
    for(int i = 0;i<memsize;i++) encmemory[i] = trlweSymEncryptlvl1(pmu[i],DEF_αbk,(*sk).key.lvl1);
    ...
    for(int i = 0; i<address_bit-1;i++){
        ...
        for(int j = 0;j<2*DEF_l;j++) for(int k = 0;k<DEF_N;k++){
            (*bootedTGSW)[1][i][j][0][k] = - (*bootedTGSW)[0][i][j][0][k];
            (*bootedTGSW)[1][i][j][1][k] = - (*bootedTGSW)[0][i][j][1][k];
        }
        for(int j = 0;j<DEF_l;j++) for(int k = 0;k<DEF_N/2;k++){
            const double doubleh = static_cast<double>(1U << (32 - (j + 1) * DEF_Bgbit));
    ...
    assert(pres == trlweSymDecryptlvl1(encmemory[intaddress], (*sk).key.lvl1)[0]);
    cout << "Passed" << endl;
    double elapsed =
