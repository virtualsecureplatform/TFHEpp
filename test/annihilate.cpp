...
int main()
{
    ...
    std::vector<std::array<uint32_t, TFHEpp::lvl1param::n>> pmu(num_test);

    for (std::array<uint8_t, TFHEpp::lvl1param::n> &i : pin)
        for (uint8_t &p : i) p = binary(engine);
    for (int i = 0; i < num_test; i++)
        for (int j = 0; j < TFHEpp::lvl1param::n; j++)
            pmu[i][j] = (pin[i][j] > 0) ? (TFHEpp::lvl1param::μ >> TFHEpp::lvl1param::nbit) : -(TFHEpp::lvl1param::μ >> TFHEpp::lvl1param::nbit);

    std::vector<TFHEpp::TRLWE<TFHEpp::lvl1param>> cin(num_test);
    for (int i = 0; i < num_test; i++)
        cin[i] = TFHEpp::trlweSymEncrypt<TFHEpp::lvl1param>(pmu[i], TFHEpp::lvl1param::α, sk->key.lvl1);
    ...
    for (int test = 0; test < num_test; test++) {
        TFHEpp::AnnihilateKeySwitching<TFHEpp::lvl1param>(cres[test], cin[test], *ahk);
    }
