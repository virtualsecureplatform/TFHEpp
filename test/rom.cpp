...
void UROMUX(TRLWElvl1 &res,const array<TRGSWFFTlvl1,address_bit> &address,array<TRLWElvl1,1<<(address_bit-width_bit)> &data){
    ...
    for(uint32_t index = 0;index<num_trlwe;index++){
        CMUXFFTlvl1(temp[index],address[width_bit],data[2*index+1],data[2*index]);
    }
    ...
    for(uint32_t bit = 0;bit<(address_bit-width_bit-2);bit++){
        ...
        for(uint32_t index = 0;index<(num_trlwe>>(bit+1));index++){
            CMUXFFTlvl1(temp[(2*index+1)*stride + offset],address[width_bit+bit+1],temp[(2*index+1)*stride + offset],temp[(2*index)*stride + offset]);
        }
    }
    const uint32_t stride = 1<<(address_bit-width_bit-2);
    const uint32_t offset = (1<<(address_bit-width_bit-2)) - 1;
    CMUXFFTlvl1(res,address[address_bit - 1],temp[stride + offset],temp[offset]);
}

template<uint32_t address_bit, uint32_t width_bit>
void LROMUX(vector<TLWElvl0> &res, const array<TRGSWFFTlvl1,address_bit> &address, const TRLWElvl1 &data, const KeySwitchingKey &ksk){
    TRLWElvl1 temp,acc;
    ...
    for(int i = 0;i<width;i++) IdentityKeySwitchlvl10(res[i],reslvl1[i],ksk);
}
...
int main()
{
    ...
    const uint32_t width_bit = DEF_Nbit - words_bit; //log_2 of how many words are in one TRLWElvl1 message.
    ...
    for (int i = 0; i < num_trlwe; i++)
        for (int j = 0; j < DEF_N; j++) pmu[i][j] = pmemory[i][j] ? DEF_μ : -DEF_μ;
    ...
    for(int i = 0;i<num_trlwe;i++) encmemory[i] = trlweSymEncryptlvl1(pmu[i],DEF_αbk,(*sk).key.lvl1);

    chrono::system_clock::time_point start, end;
    start = chrono::system_clock::now();
    for(int i = 0; i<address_bit;i++)CircuitBootstrappingFFT(bootedTGSW[i], encaddress[i], (*ck).ck);
    TRLWElvl1 encumemory;

    UROMUX<address_bit,width_bit>(encumemory,bootedTGSW,encmemory);
    LROMUX<address_bit,width_bit>(encres,bootedTGSW,encumemory,(*ck).gk.ksk);
    end = chrono::system_clock::now();

    pres = bootsSymDecrypt(encres, *sk);
    uint32_t uaddress = 0;
    uint32_t laddress = 0;
    for(int i = 0;i<(address_bit-width_bit);i++) uaddress += address[i+width_bit]<<i;
    array<bool,DEF_N> umemory;
    umemory = trlweSymDecryptlvl1(encumemory,(*sk).key.lvl1);

    for(int i = 0;i<width_bit;i++) laddress += static_cast<uint32_t>(address[i])<<(i+words_bit);
    for (uint32_t i = 0; i < (1<<words_bit); i++) assert(static_cast<int>(pres[i]) == static_cast<int>(pmemory[uaddress][laddress+i]));
    cout << "Passed" << endl;
    double elapsed =
