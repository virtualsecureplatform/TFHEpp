...
int main()
{
    ...
    TFHEpp::annihilatekeygen<TFHEpp::lvl1param>(
        *ahk, sk->key.getSubset<TFHEpp::lvl1param>());

    std::chrono::system_clock::time_point start, end;
    start = std::chrono::system_clock::now();

    for (int test = 0; test < num_test; test++) {
        TFHEpp::TLWE2TRLWEPacking<TFHEpp::lvl1param>(cres[test], ca[test],
                                                     *ahk);
    }
    ...
    for (int i = 0; i < num_test; i++) {
        std::array<bool, TFHEpp::lvl1param::n> pres =
            TFHEpp::trlweSymDecrypt<TFHEpp::lvl1param>(
                cres[i], sk->key.getSubset<TFHEpp::lvl1param>());
