...
void FFT_Processor_FFTW::execute_direct_torus32(uint32_t *res, const double *a)
{
    ...
    for (int i = 0; i < Ns2; i++) {
        std::complex<double> tmp = twist[i] * std::complex(a[i], a[Ns2 + i]);
        in[i][0] = tmp.real();
        in[i][1] = tmp.imag();
    }
...
void FFT_Processor_FFTW::execute_direct_torus64(uint64_t *res, const double *a)
{
    ...
    const uint64_t *const vals = (const uint64_t *)out;
    for (int i = 0; i < Ns2; i++) {
        for (int j = 0; j < 2; j++) {
            uint64_t val =
                (vals[i * 2 + j] & valmask0) | valmask1;  // mantissa on 53 bits
            uint16_t expo =
                (vals[i * 2 + j] >> 52) & expmask0;  // exponent 11 bits
            // 1023 -> 52th pos -> 0th pos
            // 1075 -> 52th pos -> 52th pos
            int16_t trans = expo - 1075;
            uint64_t val2 = trans > 0 ? (val << trans) : (val >> -trans);
            res[i + Ns2 * j] = (vals[i * 2 + j] >> 63) ? -val2 : val2;
        }
    }
