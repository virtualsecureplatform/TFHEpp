...
void HalfAdder(TFHEpp::TLWElvl0& sum, TFHEpp::TLWElvl0& carry, const TFHEpp::TLWElvl0& ca, const TFHEpp::TLWElvl0& cb, const TFHEpp::GateKey& gk){
    HomXOR(sum,ca,cb,gk);
...
//   algo: sum and carry
void FullAdder(TFHEpp::TLWElvl0& sum, TFHEpp::TLWElvl0& carry, const TFHEpp::TLWElvl0& cai, const TFHEpp::TLWElvl0& cbi, const TFHEpp::TLWElvl0& cci, const TFHEpp::GateKey& gk) {
...
int main(){
    ...
    for (int i; i<16; i++) {
        if((cloud_input>>i)&1) TFHEpp::HomCONSTANTONE(cloudcipher[i]);
        else TFHEpp::HomCONSTANTZERO(cloudcipher[i]);
    }
    ...
    for (int i; i<16; i++) {
        ...
        FullAdder(tmpsum,flagcloud,clientcipher[i],cloudcipher[i],flagcloud,*gk);
